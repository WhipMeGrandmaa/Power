...
public class SellMenu extends Menu {
	...
	public SellMenu(Player person) {
		...
		this.setTitle("&ePower Sell Shop");

		this.spiderEye = Button.makeSimple(ItemCreator.of(CompMaterial.SPIDER_EYE, "&dSpider Eye", "&fPower per 1: &e" + SellPriceFile.spiderEye, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.spiderEye, CompMaterial.SPIDER_EYE, this);
				});

		this.netherWart = Button.makeSimple(ItemCreator.of(CompMaterial.NETHER_WART, "&dNether Wart", "&fPower per 1: &e" + SellPriceFile.netherWart, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.netherWart, CompMaterial.NETHER_WART, this);
				});

		this.amethyst = Button.makeSimple(ItemCreator.of(CompMaterial.AMETHYST_SHARD, "&dAmethyst", "&fPower per 1: &e" + SellPriceFile.amethyst, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.amethyst, CompMaterial.AMETHYST_SHARD, this);
				});

		this.rottenFlesh = Button.makeSimple(ItemCreator.of(CompMaterial.ROTTEN_FLESH, "&dRotten Flesh", "&fPower per 1: &e" + SellPriceFile.rottenFlesh, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.rottenFlesh, CompMaterial.ROTTEN_FLESH, this);
				});

		this.lives = Button.makeSimple(ItemCreator.of(CompMaterial.ENCHANTED_GOLDEN_APPLE, "&dLives", "&fPower per 1: &e" + SellPriceFile.lives, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.lives, CompMaterial.ENCHANTED_GOLDEN_APPLE, this);
				});

		this.mobFragments = Button.makeSimple(ItemCreator.of(CompMaterial.PHANTOM_MEMBRANE, "&dMob Fragments", "&fPower per 1: &e" + SellPriceFile.mobFragments, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.mobFragments, CompMaterial.PHANTOM_MEMBRANE, this);
				});

		this.magmaCream = Button.makeSimple(ItemCreator.of(CompMaterial.MAGMA_CREAM, "&dMagma Cream", "&fPower per 1: &e" + SellPriceFile.magmaCream, "", "&f&nClick to sell all."),
				player -> {
					PowerManager.sell(player, SellPriceFile.magmaCream, CompMaterial.MAGMA_CREAM, this);
				});

		this.blazeRod = Button.makeSimple(ItemCreator.of(CompMaterial.BLAZE_ROD, "&dBlaze Rod", "&fPower per 1: &e" + SellPriceFile.blazeRod, "", "&f&nClick to sell all."),
				player -> {
